...
## 68. Marketplace Connections

Модуль Marketplace Connections отвечает за подключение маркетплейсов к SellerHUB.
...
## 69. Поддерживаемые маркетплейсы

| Маркетплейс | Тип | Режим в demo |
|---|---|---|
| Яндекс Маркет | `YANDEX_MARKET` | Mock/API-ready |
| Wildberries | `WILDBERRIES` | Mock/API-ready |
| Avito | `AVITO` | Mock/API-ready |
| Ozon | `OZON` | Mock/API-ready |

---

## 70. Почему используется Mock/API-ready подход

У проекта нет реальных кабинетов продавца и API-ключей, поэтому интеграции реализованы в режиме `MOCK`.

При этом архитектура подготовлена к реальным API:
...
Это позволяет демонстрировать бизнес-логику без зависимости от внешних сервисов.

---

## 71. Структура Marketplaces Module

| Файл | Назначение |
|---|---|
| `src/routes/marketplace.routes.ts` | HTTP-маршруты маркетплейсов |
| `src/controllers/MarketplaceController.ts` | Обработка HTTP-запросов |
| `src/services/MarketplaceService.ts` | Бизнес-логика подключений |
